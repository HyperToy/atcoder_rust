    ...
    for _ in 0..q {
        ...
        match t {
            1 => {
                ...
                boxes.entry(j).or_insert(MultiSet::new()).insert(i);
                cards.entry(i).or_insert(BTreeSet::new()).insert(j);
            }
            2 => {
                input! { i: usize }
                answer.push(boxes.entry(i).or_insert(MultiSet::new()).iter().join(" "));
            }
            3 => {
                input! { i: usize }
                answer.push(cards.entry(i).or_insert(BTreeSet::new()).iter().join(" "));
            }
            _ => unreachable!(),
        }
    }
