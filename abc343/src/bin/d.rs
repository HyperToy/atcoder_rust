...
impl Solver {
    fn new(n: usize) -> Self {
        let points = vec![0; n];
        let set = MultiSet::from(points.clone());
        Self { points, set }
    }
    fn query(&mut self, a: usize, b: i64) -> usize {
        self.set.remove(&self.points[a]);
        self.points[a] += b;
        self.set.insert(self.points[a]);
        self.set.len()
    }
}
