...
fn main() {
    // これが最大費用流・最小カットで解けるのがわからん。
    input! {
        n: usize, m: usize,
        profits: [i32; n],
        requests: [(Usize1, Usize1); m],
    }
    let mut maximum_flow = MaximumFlow::new(n + 2);
    let mut offset = 0;
    for i in 0..n {
        let p = profits[i];
        if p >= 0 {
            maximum_flow.add_edge(n, i, p);
            offset += p;
        } else {
            maximum_flow.add_edge(i, n + 1, -p);
        }
    }
    for (u, v) in requests {
        maximum_flow.add_edge(u, v, 1_000_000_000);
    }
    println!("{}", offset - maximum_flow.max_flow(n, n + 1));
}
